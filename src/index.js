...
function sanitizeSdp(sdp) {
    ...
    // Some Brave builds reject ulpfec (FEC) and red (redundant encoding) codec
    // lines in offers generated by Brave incognito. Collect their dynamic payload
    // types so we can strip all related lines AND remove them from m= lines.
    const badPt = new Set()
    for (const l of lines) {
        const m = l.match(/^a=rtpmap:(\d+) (?:ulpfec|red)\//)
        if (m) badPt.add(m[1])
    }
    // Also collect RTX payload types that retransmit a bad codec (apt=<bad>)
    for (const l of lines) {
    ...
    return lines
        ...
        .map(l => {
            // Strip bad payload types from the m= codec list
            if (!l.startsWith('m=')) return l
            const parts = l.split(' ')
            // m= format: m=<type> <port> <proto> <pt1> <pt2> ...
